  ...
  )
{
  ...
  B1Center->vars[0] = 0.5*(B1Left.vars[0]   + B1Right);
  B2Center->vars[0] = 0.5*(B2Bottom.vars[0] + B2Top  );
  B3Center->vars[0] = 0.5*(B3Back.vars[0]   + B3Front);
}
...
void timeStepper::computeDivB(const grid &B1Left,
                              const grid &B2Bottom,
                              const grid &B3Back,
                              int &numReads,
                              int &numWrites
